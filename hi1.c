...
void pass2()
{
    ...
    int i=0,j,k,c,t=0,size=0,start,final,l,ctr[2]={0X0};
   ...
   fp4=fopen("intermediate.txt","r");
    ...
    fprintf(fp5,"T^00%s^%X",counter,ctr[0]);
    fclose(fp5);
    while(strcmp(arr4,"END")!=0)
     {
       ...
       else
       {
           ...
           {
               while(strcmp(arr5,"RESW")==0||strcmp(arr5,"RESB")==0)
               {
                 fscanf(fp4,"%s\t%s\t%s\t%s",counter,arr4,arr5,arr6);
               }
               fp5=fopen("output3.txt","a");
               fprintf(fp5,"\n");
               fprintf(fp5,"T^%s^%X",counter,ctr[1]);
               fclose(fp5);
